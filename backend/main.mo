...
actor PayPact {
  ...
  public shared ({ caller }) func summary() : async [UserSummary] {
    ...
    for ((_, user) in users.entries()) {
      ...
      for ((_, bill) in bills.entries()) {
        if (bill.participants.find(func(p : Principal) : Bool { p == user.id }) != null) {
  ...
  transient let guards = {
    requireAuth = func(caller : Principal) {
